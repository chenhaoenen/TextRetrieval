...
def parse_arguments():
    ...
    parser.add_argument('--triple_text_with_label_path', required=True, type=str)
...
def triples_data_loader(args, tokenizer):
    ...
    #tiples
    args.train_data_num = int(subprocess.check_output("wc -l " + args.triple_text_with_label_path, shell=True).split()[0])
    ...
    class TriplesDataset(IterableDataset):
        ...
        def line_mapper(self, line):
            query, passage, label = line.strip().split('\t')
            return query, passage, float(label)
    ...
    train_data_loader = DataLoader(dataset=TriplesDataset(args.triple_text_with_label_path),
