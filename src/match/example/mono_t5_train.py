...
from ..model.mono_t5 import MonoT5
from src.utils.timer import stats_time
from torch.utils.data import IterableDataset, DataLoader
from transformers import T5Tokenizer, set_seed, logging
...
def prepare_model_and_optimizer(args, device):
    ...
    model = MonoT5(args.pretrained_model_path)
    model.t5.to(device)
    optimizer = optim.Adam(model.t5.parameters(), lr=args.learning_rate)
...
def triples_data_loader(args, tokenizer):
    ...
    def collate_fn(batch):
        ...
        patterns = [pattern.format(query=q, document=d) for q, d in zip(querys, passages)]
        labels = ['true' if l == 1.0 else 'false' for l in labels]
        patterns_tokenize = tokenizer(patterns, max_length=args.max_seq_length, truncation=True, padding='max_length', return_tensors='pt')
        labels_tokenize = tokenizer(labels, return_tensors='pt')

        return patterns_tokenize, labels_tokenize
...
#
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(train_data_loader):
            ...
            loss = model.train(patterns=patterns, labels=labels)
